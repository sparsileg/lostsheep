...
fn run_diff(
    ...
    // #NN: every existing household that got a real disposition this batch
    // (exact source_key match — changed or unchanged — or a name-match
    // pairing) is recorded here. The removed-scan below must skip these
    // regardless of source_key membership: an address change alone makes
    // the OLD source_key vanish from seen_source_keys even though the
    // household was just matched and handled by name, which previously
    // caused the same household to show up as both 'changed' and
    // 'removed' in Review Updates.
    let mut matched_existing_ids: std::collections::HashSet<i64> = std::collections::HashSet::new();
        ...
        if matching_ids.len() == 1 {
            ...
            if coords_drifted(existing_lat, existing_lon, rec.latitude, rec.longitude) {
                ...
                if let Some(id) = name_match {
                    matched_existing_ids.insert(id);
                }
    ...
    // ceiling that's ~10^8 comparisons on a full re-import.
    {
        ...
        for (id, key) in existing_keys {
            if !seen_source_keys.contains(&key) && !matched_existing_ids.contains(&id) {
                tx.execute(
...
fn auto_accept_all(
    ...
    for (i, rec) in incoming.iter().enumerate() {
        ...
        let source_key_seq: i64 = tx
            ...
            .map_err(|e| e.to_string())?;
        // Issue #69: a genuinely new household — mint its durable identity
        // here, once, never regenerated by any later Replace/Merge.
        let household_uid = Uuid::new_v4().to_string();
        tx.execute(
            ...
            params![
                household_uid, rec.first_name, rec.last_name, rec.role, rec.phone_1, rec.email_1, rec.first_name_2, rec.last_name_2, rec.role_2,
        ...
        "add" | "replace" | "merge" => {
            ...
            if action == "replace" || action == "merge" {
                if let Some(old_id) = existing_id {
                    ...
                    // sentinel bump / delete below, same as
                    // existing_comments above.
                    carried_household_uid = tx
                        .query_row("SELECT household_uid FROM households WHERE id = ?1", params![old_id], |r| r.get(0))
                        .optional()
                        .map_err(|e| e.to_string())?;
                    ...
                    //
                    // Same problem, same fix, for household_uid: the new
                    // row below is inserted carrying this old row's uid
                    // forward (carried_household_uid, read above) while
                    // the old row — still holding that same uid — hasn't
                    // been deleted yet. That collided with the UNIQUE
                    // index on households.household_uid. Bump the old
                    // row's uid to a disposable throwaway value in the
                    // same UPDATE, freeing the real uid for the insert.
                    let temp_uid = format!("superseded-{}", Uuid::new_v4());
                    tx.execute(
                        "UPDATE households SET source_key_seq = -id, household_uid = ?2 WHERE id = ?1",
                        params![old_id, temp_uid],
                    )
                    .map_err(|e| e.to_string())?;
                }
            }
