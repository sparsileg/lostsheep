...
CREATE TABLE IF NOT EXISTS households (
    ...
    -- Durable identity (issue #69): a UUIDv4 minted once when a household
    -- first enters the database (auto_accept_all / resolve_review_item's
    -- "add" path) and carried forward unchanged by Replace/Merge — unlike
    -- id (a rowid, regenerated by Replace/Merge) or source_key (derived
    -- from mutable name/address fields, and not even unique alone — see
    -- source_key_seq's own comment below). Not UNIQUE inline, same reason
    -- as tags.system_key above it in this file: an existing database
    -- needs this column added via ALTER TABLE first (see
    -- db/mod.rs::migrate_household_uid), and SQLite's ALTER TABLE ADD
    -- COLUMN refuses a UNIQUE column outright. Uniqueness comes from the
    -- separate index below instead, so fresh and migrated databases end
    -- up structurally identical.
    household_uid TEXT NOT NULL,
...
-- Issue #69 — see household_uid's column comment above.
...
CREATE TABLE IF NOT EXISTS deleted_households (
    ...
    -- Issue #69 — mirrored from households.household_uid at delete time
    -- and carried back unchanged by restore_deleted_household(). Nullable
    -- here (unlike households.household_uid): a row soft-deleted before
    -- this column existed has no value to backfill from, and this is a
    -- mirror table, not the identity's source of truth — no UNIQUE
    -- constraint needed on a table rows only pass through.
    household_uid    TEXT,
    -- Issue #61, Piece 2 — the original source_key_seq, discarded
    -- entirely before this column existed (Piece 1 of #61 already
    -- restores successfully without it, via MAX+1). When present,
    -- restore prefers this exact value if it's still free and falls back
    -- to MAX+1 only if something else has since taken it. NULL for any
    -- row soft-deleted before this column existed, or before soft_delete
    -- _household() was updated to populate it — restore always falls
    -- back to MAX+1 for those, same as it does today.
    source_key_seq   INTEGER,
    first_name       TEXT NOT NULL,
    last_name        TEXT NOT NULL,
    role             TEXT NOT NULL,
    phone_1          TEXT,
...
CREATE TABLE IF NOT EXISTS visits (
    id           INTEGER PRIMARY KEY,
    -- Issue #69/#70 — a UUIDv4 minted once per visit at record_visit()
    -- time. Not needed for this issue's own identity problem (visits
    -- already belong to a household via household_id, which is what
    -- #69 fixes); exists so #70's recovery journal can dedupe replayed
    -- events without a second migration touching this same table later.
    -- Not UNIQUE inline, same ALTER TABLE restriction as household_uid
    -- above — see db/mod.rs::migrate_visit_uid.
...
-- Issue #69/#70 — see visit_uid's column comment above.
...
CREATE TABLE IF NOT EXISTS deleted_visits (
    ...
    -- Issue #69/#70 — mirrored from visits.visit_uid at delete time,
    -- carried back by restore_deleted_household(). Nullable, same
